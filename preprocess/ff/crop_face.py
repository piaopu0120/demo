...
def facecrop(model_det,video_path,save_path,num_frames=10,scale = 1.7,connection = None):
    ...
    bbox_dir = save_path + 'bbox_'+str(num_frames)+'/'
    ...
    for frame_idx in range(frame_count):
        ...
        # face_dir=save_path+'faces_'+str(num_frames)+'/'+os.path.basename(video_path).replace('.mp4','/') 
        # print(face_dir)
...
def generate_frame(video_path,save_path,num_frames=10):
    capture = cv2.VideoCapture(video_path)
    frame_count =int(capture.get(cv2.CAP_PROP_FRAME_COUNT))
...
def crop_face_from_bbox(target_path,num_frames=10):
    target_frame_dir = target_path + 'frames_'+str(num_frames)+'/'
    ori_bbox_dir = target_path + 'bbox_'+str(num_frames)+'/'
    target_face_dir = target_path + 'faces_'+str(num_frames)+'/'
    os.makedirs(target_face_dir,exist_ok=True)
    target_video_dirs = sorted(glob(target_frame_dir+'*/'))
    for target_video_dir in target_video_dirs:
        splits = target_video_dir.split('/')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d',dest='dataset',default='FaceShifter',choices=['Original','Deepfakes', 'Face2Face', 'FaceSwap', 'NeuralTextures','FaceShifter']) 
    ...
    parser.add_argument('-s',dest='scale',type=float,default=1.5)
   # FaceShifter
    ...
    for args.dataset in ['Original','Deepfakes', 'Face2Face', 'FaceSwap', 'NeuralTextures','FaceShifter']:
        ...
        crop_face_from_bbox(dataset_path,args.num_frames)
    ...
    # else:
    #     for i in tqdm(range(n_sample)):
    #         generate_frame(video_path_list[i],save_path=dataset_path,num_frames=args.num_frames)
